...
@Getter
public class CommandBlock extends MetaMapper {
    ...
    private final Map<Boolean, Integer> conditional = new HashMap<>() {{
        put(false, 0);
        put(true, 8);
    }};
    ...
    public int getMeta(NbtMap originalState) {
        return originalState.getInt("facing_direction")
                + conditional.get(originalState.getBoolean("conditional_bit"));
    }
}
