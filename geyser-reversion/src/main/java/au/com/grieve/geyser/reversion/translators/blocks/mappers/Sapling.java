...
@Getter
public class Sapling extends MetaMapper {
    ...
    private final Map<Boolean, Integer> age = new HashMap<>() {{
        put(false, 0);
        put(true, 8);
    }};
    ...
    public int getMeta(NbtMap originalState) {
        return material.get(originalState.getString("sapling_type"))
                + age.get(originalState.getBoolean("age_bit"));
    }
}
