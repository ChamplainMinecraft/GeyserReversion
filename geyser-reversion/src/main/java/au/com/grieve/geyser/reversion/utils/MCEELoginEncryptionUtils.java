...
package au.com.grieve.geyser.reversion.utils;

import au.com.grieve.geyser.reversion.EduSupportPlugin;
import au.com.grieve.geyser.reversion.data.BedrockClientData;
...
public class MCEELoginEncryptionUtils {
    ...
    private static void encryptConnectionWithCert(GeyserConnector connector, GeyserSession session, String clientData, JsonNode certChainData) {
        try {
            ...
            session.setClientData(JSON_MAPPER.convertValue(jsonData, BedrockClientData.class));
    ...
    public static void startEncryptionHandshake(GeyserSession session, PublicKey key, String tenantId) throws Exception {
        ...
        if (tenantId != null) {
            if (EduSupportPlugin.getInstance().getTokenManager().getTokenMap().containsKey(tenantId)) {
                // This may block if it needs to be refreshed
                signedToken = EduSupportPlugin.getInstance().getTokenManager().getTokenMap().get(tenantId).getSignedToken();
            } else {
